...
const argv = yargs
  ...
  .help(false)
  .version(false)
  .epilog('Serverless deployment of Angular apps using ServeRX-ts')
  .argv;
...
const fileName = <string>argv.deploy;
console.log(chalk.blueBright(`Loading deployment file ${fileName}...`));
const deployment: Deployment = loadDeployment(fileName, errors, warnings, infos);

const appDir = <string>argv.app;
...
// all the app files
files.forEach(f => fs.copyFileSync(f, path.join(tmpDir.name, f.substr(appDir.length))));
...
fs.writeFileSync(path.join(tmpDir.name, 'serverless.yml'), yaml.safeDump(serverless));
