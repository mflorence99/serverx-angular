...
describe('loadApp unit tests', () => {
  ...
  test('errors are generated for an invalid app directory', () => {
    ...
    expect(errors.length).toEqual(1);
    expect(errors).toEqual(expect.arrayContaining([config.messages.MISSING_INDEX_HTML]));
  });
...
describe('loadDeployment unit tests', () => {
  ...
  test('a valid AWS deployment file is loaded successfully', () => {
    const errors = [], warnings = [], infos = [];
    const deployment = loadDeployment('./sample/deploy/aws.json', errors, warnings, infos);
    expect(deployment.app).toEqual('serverx-angular');
    expect(deployment.environment.BACKEND_ENDPOINT).toEqual('https://admin.appcast.cloud');
    ...
    expect(deployment.tenant).toEqual('mflorence99');
    expect(errors.length).toEqual(0);
    expect(warnings.length).toEqual(0);
  });
  ...
  test('a valid Google deployment file is loaded successfully', () => {
    const errors = [], warnings = [], infos = [];
    const deployment = loadDeployment('./sample/deploy/google.json', errors, warnings, infos);
    expect(deployment.app).toEqual('serverx-angular');
  ...
  test('errors are generated for an invalid deployment file', () => {
    const errors = [], warnings = [], infos = [];
    const deployment = loadDeployment('./sample/deploy/invalid.yml', errors, warnings, infos);
    expect(deployment).toBeDefined();
    expect(errors.length).toBeGreaterThan(0);
    expect(errors).toEqual(expect.arrayContaining([config.messages.BAD_PROVIDER]));
    expect(errors).toEqual(expect.arrayContaining([config.messages.MISSING_APP]));
    expect(errors).toEqual(expect.arrayContaining([config.messages.MISSING_REGION]));
    expect(errors).toEqual(expect.arrayContaining([config.messages.MISSING_SERVICE]));
    expect(errors).toEqual(expect.arrayContaining([config.messages.MISSING_TENANT]));
  });

  test('errors are generated for an invalid AWS deployment file', () => {
    const errors = [], warnings = [], infos = [];
    const deployment = loadDeployment('./sample/deploy/invalid-aws.yml', errors, warnings, infos);
    expect(deployment).toBeDefined();
    expect(errors.length).toBeGreaterThan(0);
    expect(errors).toEqual(expect.arrayContaining([config.messages.MISSING_STAGE]));
  });

  test('errors are generated for an invalid Google deployment file', () => {
    const errors = [], warnings = [], infos = [];
    const deployment = loadDeployment('./sample/deploy/invalid-google.yml', errors, warnings, infos);
    expect(deployment).toBeDefined();
    expect(errors.length).toBeGreaterThan(0);
    expect(errors).toEqual(expect.arrayContaining([config.messages.MISSING_CREDENTIALS]));
    expect(errors).toEqual(expect.arrayContaining([config.messages.MISSING_PROJECT]));
  });
 ...
 */
  ...
  describe('loadServerless unit tests', () => {
    ...
    test('properly configure serverless.yml from AWS deployment', () => {
      ...
      expect(serverless.app).toEqual(deployment.app);
      expect(serverless.tenant).toEqual(deployment.tenant);
